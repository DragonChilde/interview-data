...
`CountDownLatch`主要有两个方法，当一个或多个线程调用await方法时，调用线程就会被阻塞。其它线程调用`CountDown`方法会将计数器减1（调用`CountDown`方法的线程不会被阻塞），当计数器的值变成零时，因调用await方法被阻塞的线程会被唤醒，继续执行
...
最后我们需要通过`CountDownLatch`的await方法来控制班长主线程的执行，这里`countDownLatch.await()`可以想成是一道墙，只有当计数器的值为0的时候，墙才会消失，主线程才能继续往下执行
...
引入`CountDownLatch`后的执行结果，我们能够控制住main方法的执行，这样能够保证前提任务的执行
...
集齐7个龙珠，召唤神龙的Demo，我们需要首先创建`CyclicBarrier`
